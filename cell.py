...
class Cell:
    ...
    def draw(self, x1, y1, x2, y2, debug=False):
        ...
        if self.has_top_wall:
            self._win.draw_line(Line(Point(x1, y1), Point(x2, y1)), self._color)
        else:
            self._win.draw_line(Line(Point(x1, y1), Point(x2, y1)),'white')
        if self.has_left_wall:
            self._win.draw_line(Line(Point(x1, y1), Point(x1, y2)), self._color)
        else:
            self._win.draw_line(Line(Point(x1, y1), Point(x1, y2)),'white')
        if self.has_right_wall:
            self._win.draw_line(Line(Point(x2, y1), Point(x2, y2)), self._color)
        else:
            self._win.draw_line(Line(Point(x2, y1), Point(x2, y2)),'white')
        if self.has_bottom_wall:
            self._win.draw_line(Line(Point(x1, y2), Point(x2, y2)), self._color)
        else:
